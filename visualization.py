...
def visualizeDate(daily: DailyNutritionRecord):
    pieChartData = {}
    for attr, value in vars(daily.totalNutritionInfo).items():
        if(attr == 'calories'): continue
        pieChartData[attr] = value

    
    data = pd.Series(pieChartData).reset_index(name='value').rename(columns={'index': 'macros'})
    data['angle'] = data['value']/data['value'].sum() * 2*pi
    data['color'] = Category20c[len(pieChartData)]
    ...
    x = [daily.date]
    y = [daily.totalNutritionInfo.calories]
